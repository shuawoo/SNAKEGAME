...
def draw_snake(color, object):
    '''draw Snake'''
    pygame.draw.rect(scrn, green, (x1, y1, snake_block, snake_block))

def draw_fruit(fruit):
    '''draw Fruit'''
    pygame.draw.circle(scrn, orange, (fruit.x, fruit.y),snake_block)
...
def gameloop():
    ...
    x1 = round(random.randrange(10, scrn_width - snake_block) / 10.0) * 10.0
    y1 = round(random.randrange(10, scrn_height - snake_block) / 10.0) * 10.0
    snake_head = snake(x1, y1)
    snake_body = [snake(x1, y1), snake(x1, y1)]
    ...
    while not game_over:
        ...
        if x1 >= scrn_width or x1 <= 0 or y1 >= scrn_height or y1 < 0:  # !! music?
            game_close = True
